...
import { toggleGptSearch, showGptSearch } from "../../store/slices/gptSlice";
    ...
    return (
        <header className="fixed top-0 w-full px-4 md:px-16 py-4 flex flex-col md:flex-row items-center justify-between z-50 bg-linear-to-b from-black/90 via-black/50 to-transparent transition-all duration-300">
            ...
            <div className="hidden md:flex items-center gap-6 text-white text-sm">
                ...
                {/* AI Search Button */}
                <button className="w-20 bg-purple-500 text-white rounded p-1 font-bold" onClick={handlegptSearch}>
                    {isGptSearchVisible ? "Movies Page" : "AI Search"} 
                </button>
                ...
                <div className="flex items-center gap-2 cursor-pointer group" onClick={handleSignOut}>
                    ...
                    />
                    {/* Caret Down Icon */}
                    <svg className="w-4 h-4 transition-transform group-hover:rotate-180 duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
